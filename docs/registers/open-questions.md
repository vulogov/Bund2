...
| Q20 | F33: `PartialEq` is asymmetric across int/float — `42 == 42.5` truncates and answers true, `42.5 == 42` widens and answers false (`reference/rust_dynamic/src/eq.rs:13,24`). D30 hashes by content, so a bucket assignment must decide whether `42` and `42.5` collide, and **no assignment is consistent with an asymmetric equality**. Which direction does Bund2 take — truncate, widen, or make the pair unequal both ways? Each is a deviation. | F33, D30 |
