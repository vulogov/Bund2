...
| Q19 | D30 makes `valuemap` readable through `get` but does not name `?key`. It needs the same fix in both layers: the word stringifies the key (`reference/rust_multistackvm/src/stdlib/values/value_dict.rs:97`), and `Value::has_key` omits `VALUEMAP` from its arm while its catch-all answers false (`reference/rust_dynamic/src/has_key.rs:7,19-20`). `set` has no `?key` counterpart to mirror, so the decision's shape does not extend mechanically. Does `?key` get the same branch? | D30 |
| Q18 | **REOPENED.** Closed as "grounded — `q` is constant in reach" on a scan that never opened `reference/rust_dynamic/src/q.rs`. `q` has two writers, `calc_q` (`:5`) and `set_q` (`:9`), reached from `impl Add`/`Sub`/`Mul`/`Div` (`reference/rust_dynamic/src/math.rs:416-420`), so **every arithmetic operation writes it**. The goldens show 100.0 because it is a fixpoint — all constructors start there and the average of 100.0 with itself is 100.0 — not because it is constant. And the fixpoint is escapable in scope: `Value::none` is `Value::new` with `q: 0.0` (`reference/rust_dynamic/src/create_special.rs:19-21`, `value.rs:38`), and the JSON converter returns it for a null (`reference/rust_dynamic/src/cast_json_to_value.rs:39`). RFC-0001 now carries `q` as a field. What remains open is whether any *word* can observe a `q` other than 100.0, which needs a probe rather than a scan | RFC-0001 |
