...
| Q39 | **RESOLVED as D68, 2026-09-14 — option (a), `produces == 0`.** Two corrections to the filing below, both of which narrow it. The model stated here is too strong: promoted values need not be a *suffix* of the abstract stack, only in *order* relative to what the real stack holds — so `1 2 3 f` may push `3` alone while `1` and `2` stay in registers. What breaks is the **final sync**, because pushing only appends, so promoted values must be the top of the abstract stack when synced and a producing callee leaves its result above them permanently. And the cost is smaller than "most of `PROMOTABLE.txt`'s arithmetic": **56 of its 222 natives survive**, and they are the mid-run effectful words — `println`, `drop`, `return`, `to_stack`, `register`, `alias`, the `.`-suffixed workbench family. The arithmetic the rule excludes is what *inlining* takes, where there is no call to cross. D68 records the rejected options and why criterion 10's measurement (chaining bought nothing) argues for the cheapest one. Original filing follows. **Promotion across a call is unsound for a callee that produces output, and §S5 does not say so.** §S5 keeps "the values below the callee's arity" in registers across a call. With the model promotion actually has — the abstract stack is *the real stack* followed by *the promoted values*, which are the topmost — that holds only when the callee's `produces` is **0**. Take `1 2 3 f` with `f` at `eff(1, 1)`: `3` is pushed, `f` consumes it and leaves `r`, so the real stack is `[…deep…, r]`, and syncing `1` and `2` afterwards gives `[…deep…, r, 1, 2]` where the order must be `[…deep…, 1, 2, r]`. Wrong silently, and no guard catches it — the same shape as the excess-sync case one level up. **Every worked example in the RFC sits on the safe side by accident**: D46's is `1 2 3 f` with `f` inferring as `(1, 0)`, and §S5's `1 2 "s" to_stack +` uses `to_stack` at `eff(1, 0)`. Under today's int-only register file the options are (a) promote across a call only when `produces == 0`, which excludes most of `PROMOTABLE.txt`'s arithmetic since those are `(2, 1)`; (b) pull the callee's outputs back into registers after the call, impossible while a register holds an `i64` and a produced value may be any type; or (c) widen the register file to hold a `BundValue`, which is a different feature. **Not blocking the resume index**: chaining across *inlined sites* needs no rule here, because a failing guard leaves the fast path for the residual rather than merging a divergent model. Raised while building D66's successor, 2026-09-14 | RFC-0005 §S5 |
...
| Q34 | **Answered 2026-09-11 by the owner: derive the set by audit (option 2) — recorded as D55.** *As raised:* **What identifies a word that reads the stack beyond its arity?** `debug.display_stack` declares `eff(0, 0)` and calls `vm.snapshot()`, reading everything; it runs in the golden capture epilogue. More broadly most natives guard on `vm.depth()`, which promotion makes short, so a correctly-declared `Fixed(2, 1)` word can still answer `Stack is too shallow` where Tier 0 answers normally. `StackEffect` records what a word *consumes*, not what it *observes*, and nothing else records it either. Neither `Opaque` nor RFC-0005 criterion 13 covers it. **Extended 2026-09-10 (RFC-0005's sixth review, B2):** words that reach *another* stack by name have the same shape when the name is the current stack's — `swap_in`, `rotate_stack_left` and `rotate_stack_right` (`eff(1, 0)`, `crates/bund2-stdlib/src/stack.rs`) read or reorder values promotion may be holding. Words that *switch* the current stack are a different case, handled by RFC-0005 §S5's current-stack epoch rather than by this set. **Also 2026-09-10:** while the reporter wants stacks, every native that calls `Vm::report` reads the whole stack through `Interp::report`'s snapshot — the same shape again. RFC-0005 §S5 handles that case structurally, by keeping nothing promoted across a call in that mode. **Narrowed the same day by D45:** the reporter now says per severity whether it wants a snapshot, and the CLI's `TextReporter` wants one only for a fatal report, which is made after evaluation returns. So in the default configuration a mid-body warning or notice reads no stack, and the case arises only under a reporter that wants mid-body snapshots | RFC-0005 §S5, §S6, criterion 14 |
...
| Q38 | open | **What does `use` do in a built artefact?** In the interpreter it fetches a `file://` or `http://` URL and evaluates the text (D54, `reference/Bund/src/stdlib/functions/bund/bund_use.rs:31-33`). A built program is harder in two ways. The path can be a string built at run time (D16), so `bund2 build` cannot always know which files a program loads. And `--emit=bundle` must stay a self-contained, toolchain-free artefact (D10). **Options:** (1) **fetch at run time**, as the interpreter does: faithful and simple, but the artefact is not self-contained, and `file://` paths resolve on the machine it runs on. (2) **Embed at build time the files named by a `use` with a literal operand, and fall back to fetching** for anything else. The artefact is self-contained for the common case, and computed paths still work. An embedded file keeps its build-time contents, which RFC-0006 would have to state as the one change in meaning. (3) **Refuse `use` in a built artefact** unless a flag allows run-time fetching: strict and predictable, but a program that runs in the interpreter would fail when built. **Proposed, not adopted: (2).** Raised 2026-09-11 while implementing `use` | RFC-0006, D10, D16, D54 |
...
| Q22 | answered in Draft by RFC-0005 §S7 (2026-09-11 note: **two entries share the number Q22**, this one and the probe-coverage one near the top of this file; `docs/rfc/RFC-0003-syntax-ir-and-tier0.md:954` and `docs/rfc/RFC-0005-cranelift-tier.md:1417` mean this one, the compiled-cache question. Numbers are not reassigned, so cite it as "Q22 (cache)". It closes when RFC-0005 is Accepted.) | What is the compiled-cache promotion threshold and cap, and where are they stated? D3's amended resolution makes eval's tier eligibility a consequence of identity keying rather than a rule, which presumes a threshold and a bound that RFC-0005 owns. RFC-0005 must state them as load-bearing for D3, not as tuning. Raised by RFC-0003's first review |
