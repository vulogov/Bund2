...
- Decisions consumed: D3 (no eval-specific tier rule), D16 (the world is
  permanently open, so a call target may be a name computed at runtime)
...
- Supersedes: `docs/research/05-rfc-roadmap.md` §1.1's framing of control flow,
  which is refined rather than contradicted — see Current behaviour §3.
...
a program ending in a word does not parse. All four evaluation paths append
`\n` before parsing — `bundcore_eval.rs:8`, `helpers/eval.rs`,
`helpers/run_snippet.rs`, and `bund_interpreter.rs:28`. The workaround is
uniform, undocumented, and load-bearing.
...
(`reference/bund_language_parser/src/vm/ctx.rs:8-19`). The vector it mutates is
...
each push a `Value::conditional()` carrying `type`
(`reference/Bund/src/stdlib/functions/conditional/mod.rs:36-43`), and each
registers a Rust handler under that type string into `CF` (`:20-27`). `!` on a
CONDITIONAL reads `type`, looks it up, and calls the handler
...
### D2. Scoped blocks, and `ctx` stops being a side channel

`( … )` becomes a node in the AST like `{ … }` and `[ … ]`, not a mutation of
the output vector. Lowering emits the same three-part stream the reference
produces — a CONTEXT marker, the inner terms, an `endcontext` call — so the
runtime sees what it sees today. F9 closes at the representation, not at the
semantics.
...
- keyed by a **content hash** of the `Vec<BundValue>`;
- invalidated when the body changes, which under RFC-0001's `Rc::make_mut`
  semantics means a new body with a new hash rather than mutation in place;
- entirely optional — Tier 0 can interpret the `Vec<BundValue>` directly, and
  must, because that is the fallback when a body is built and run once.

D3 (resolved) consumes this directly: eval'd code is hashed like any other
body, and gets no tier rule of its own.

### D4. The flat frame loop

Tier 0 is one loop over an explicit frame stack. No Rust recursion, anywhere on
the evaluation path.
...
| `ctx` flattening via the shared vector | preserved in the emitted stream; **representation changed** (F9) |
...
| `execute.` guarding the wrong stack | **fixed** (F53) — widening only |
...
Four fixes here (F52, F53, F56, F57) are widenings or internal — none changes
an answer a passing golden already captures. F49/F50/F51 are preserved
*because* changing them would change what existing programs mean.
...
1. **Conformance strictly increases and nothing regresses.** `cargo xtask
   conform` must pass every golden it passes at 4/69 today, plus the goldens
   gated only on block syntax. `{` and `}` appear in 42 goldens; this RFC does
   not implement the words those goldens also need, so the criterion is: no
   golden that passes before this lands fails after it, and every golden whose
   only unimplemented construct was `{ }`, `[ ]` or `( )` parses without error.
2. **No Rust recursion on the evaluation path.** A self-recursive Bund word at
   depth 100,000 completes or reports a Bund-level error; it does not overflow
   the Rust stack. Checkable by test, not by inspection.
3. **The parser accepts exactly the reference's language.** A differential
   harness parses all 132 corpus programs through both front ends and compares
   the emitted value streams element by element, with no permitted difference.
   Separately, a rejection suite: `{ 1 println}`, `1_000` and `{}` must fail in
   Bund2 exactly where they fail in the oracle, and `1 2 +` with no trailing
   newline must succeed in both.
4. **The three grammar traps are pinned by test.** `1_000` errors, `007`
   yields three integers, `{}` and `[]` fail to parse — each asserted against
   the oracle's behaviour, not merely against this document.
5. **F53 and F57 are fixed and pinned.** `execute.` succeeds with a value on
   the workbench and an empty stack; a `context` whose lambda raises inside a
   `?try` leaves the interpreter on the stack it started from.
6. **One evaluator.** No second copy of the eval loop exists in the workspace;
   `bund.eval` and the script path share an entry point. Checkable by grep, and
   by a test that a fix to one is visible from the other.
7. **`cargo xtask cite` clean and `cargo xtask lint` clean**, as for every RFC.

## Open questions

- **Q21 — does any corpus program depend on `ctx` flattening?** D2 preserves
  the emitted stream, so the answer does not change the design, but it decides
  whether the AST node can ever be lowered differently. Not yet probed.
- **Q22 — what is the promotion threshold, and where is it stated?** D3's
  resolution makes eval eligibility fall out of the content-hash cache, which
  presumes a threshold and a cap that RFC-0005 owns. RFC-0005 must state them
  as load-bearing for D3, not as tuning.
- **F48 applies to criterion 1.** `conform` cannot express an approved
  deviation, and this RFC adds none that break a golden — but D30's two
  existing deviations already fail goldens permanently, so the number this
  criterion is measured against is understated by a fixed amount.
- **The read behind this RFC is complete for the front end and dispatch core,
  ...
  handlers (372 lines) were surveyed for shape but not read line by line, and
  `values/`'s `merge`, `unfold`, `listop` and `sort_lists` were enumerated by
  registration rather than read. Neither set constrains the IR's shape; both
  constrain RFC-0004's effect table. Recorded here so a later reader does not
  mistake enumeration for grounding.
