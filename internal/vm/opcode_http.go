...
func HttpGen(vm *VM, args ...interface{}) error {
	vm.Fatal("HTTP Import not implemented")
	return nil
}

func HttpExec(vm *VM, args ...interface{}) (*Elem, error) {
	vm.Fatal("HTTP Export not implemented")
	return nil, nil
}

func InitOpcodeHttp(vm *VM) {
	vm.RegisterOpcode("http", HttpParser, HttpLambda, HttpEval, HttpGen, HttpExec)
}
