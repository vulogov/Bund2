...
func MatrixWalk(vm *VM, e *Elem, f func(float64) float64) (*Elem, error) {
	if e.Type != "MAT" {
...
func DblockWalk(vm *VM, e *Elem, f func(float64) float64) (*Elem, error) {
	if e.Type != "dblock" {
	...
	for q1.Len() > 0 {
		...
		switch e1.Type {
		case "flt":
			q2.PushBack(&Elem{Type: "flt", Value: f(e1.Value.(float64))})
		case "int":
			q2.PushBack(&Elem{Type: "flt", Value: f(float64(e1.Value.(*big.Int).Int64()))})
		default:
			q2.PushBack(e1)
		}
	}
