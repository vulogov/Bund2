...
func PctElement(vm *VM, e *Elem) (*Elem, error) {
	if e.Type == "dblock" {
		eh, err := vm.GetType("PCT")
		vm.OnError(err, "Error in ml/Perceptron")
		p := eh.Factory(vm)
		pct := p.Value.(*PERCEPTRON)
		...
		for i := 0; i < q.Len(); i++ {
			...
			case "flt":
				dat, _ = v.Value.(*big.Float).Float64()
			case "SEPARATE":
				if _mod {
					_mod = false
					pct.D += 1
...
func PctUpdElement(vm *VM, e *Elem) (*Elem, error) {
	if e.Type == "PCT" {
		if !vm.CanGet() {
		...
		for j := 0; j < len(_res); j++ {
			q1.PushBack(&Elem{Type: "flt", Value: big.NewFloat(_res[j])})
		}
		return res, nil
	}
