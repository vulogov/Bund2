...
fn write_hermetic(repo: &Path, verdicts: &[Verdict]) -> Result<(), String> {
    ...
    s.push_str("#\n");
    s.push_str("# A program is listed when it passes two independent filters.\n");
    s.push_str("#\n");
    ...
    s.push_str("#    See tests/golden/UNSTABLE.txt — this one is empirical, and no\n");
    s.push_str("#    static analysis of word effects can derive it.\n");
    s.push_str("#\n");
    s.push_str("# Both classifications are derived from each word's registration site in\n");
    s.push_str("# reference/; see xtask/src/corpus/classify.rs for the rules and their\n");
    s.push_str("# citations.\n");
    s.push_str("#\n");
    s.push_str(&format!(
        "# {} of {} programs qualify: {} are hermetic, of which {} are in scope.\n",
        verdicts.iter().filter(|v| v.in_suite()).count(),
        verdicts.len(),
        verdicts.iter().filter(|v| v.hermetic).count(),
        verdicts.iter().filter(|v| v.in_suite()).count(),
    ));
