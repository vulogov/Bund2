...
/// The D14 core set, read from `docs/core-words.md`.
///
/// Generated by `cargo xtask scope --write`. Read rather than recomputed, so
/// `coverage` reports the partition D14 actually recorded rather than one it
/// derives independently — if the two ever disagree, regenerating the file is
/// what reconciles them.
fn load_core_words(repo: &Path) -> BTreeSet<String> {
    let Ok(src) = std::fs::read_to_string(repo.join("docs/core-words.md")) else {
        return BTreeSet::new();
    };
...
pub fn run_coverage(_args: &[String]) -> Result<(), String> {
    ...
    // preservation target, so the M6 figure is reported beside it.
    let core = load_core_words(&repo);
    if !core.is_empty() {
