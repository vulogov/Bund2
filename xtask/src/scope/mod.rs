...
pub fn run(args: &[String]) -> Result<(), String> {
    ...
    if write {
        ...
        let core_by: BTreeMap<String, Vec<String>> = {
            let v: Vec<&String> = recorded.core.iter().collect();
            variants::by_subsystem(&reg, &v)
        };
        let lib: Vec<&String> = in_scope.difference(&recorded.core).collect();
        let lib_by = variants::by_subsystem(&reg, &lib);
        out.push_str("## core\n\n");
        for (sub, mut ws) in core_by {
            ws.sort();
            out.push_str(&format!("- **{sub}** — {}\n", ws.join(" ")));
        }
        out.push_str("\n## library\n\n");
        for (sub, mut ws) in lib_by {
            ws.sort();
            out.push_str(&format!("- **{sub}** — {}\n", ws.join(" ")));
        }
